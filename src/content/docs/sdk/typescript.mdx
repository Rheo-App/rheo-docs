...
import type {
  ...
  RheoItemStatus,
  BatchUpsertItem,
} from '@rheo-finance/sdk'

function handleSale(event: ItemSoldEvent) {
  // event.data.externalId, event.data.salePrice, event.data.platform
}

function isAutomotivePart(
  domain: DomainObject,
): domain is DomainObjectAutomotivePart {
  return domain.domain === 'automotive_part'
}
```
...
```javascript
// CommonJS
const { RheoClient } = require('@rheo-finance/sdk')
